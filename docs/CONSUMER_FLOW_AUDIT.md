...
## 13. Non-inline receipt content: attachments and link-only receipts

A real gap surfaced after Phase 1 shipped and was tested end-to-end against a live Nylas connection: not every receipt email actually contains its own data. Several distinct patterns exist, with different levels of support:
...
**Pattern B — data inside a PDF attachment or a photographed/scanned receipt image, email body says only "your invoice is attached" / "here's a photo of your receipt."** Built and verified live. `services/emailReceiptService.js`'s `processPendingMessage` runs body-only extraction first (unchanged, keeps the common case cheap); only if that fails to yield a merchant name and total does it download up to `MAX_ATTACHMENTS_FOR_EXTRACTION` (2) PDF/image attachments under `MAX_ATTACHMENT_BYTES_FOR_EXTRACTION` (15MB each) and retry extraction with them added as native `document` (PDF) or `image` content blocks alongside the email text — one more adapter into the same `extractReceiptDataFromEmail`/`RECEIPT_SCHEMA`, not a second parser, per this document's own "add an adapter, don't fork the parser" rule. Image attachments are restricted to `ALLOWED_IMAGE_ATTACHMENT_TYPES` (png/jpeg/webp), matching `ALLOWED_SCAN_TYPES` on the direct photo-upload path — an email attachment gets exactly the same trust boundary a customer's own upload already does. Downloaded buffers are reused for the source-document storage step, never fetched twice. Every PDF/image attachment on an accepted message is archived as a `ScannedReceiptSourceDocument` (`attachment_pdf` or `attachment_image`), whether or not the fallback actually needed it, same as the email text itself. **Two real, separate bugs found and fixed in the same pass**: (1) PDF-attachment detection matched `attachment.content_type === 'application/pdf'` with strict equality — but real providers commonly report a Content-Type *with* parameters (`application/pdf; name=invoice.pdf`), which never matched, silently breaking the original "attach the merchant's PDF as this receipt's source document" feature since it was first written, for any attachment sent with a parameterized Content-Type header — not an edge case, the common case; (2) the private storage bucket only allowed image MIME types when this was built for scan photos alone, so an `attachment_image` upload would have failed the same way the original `email_text`/PDF uploads did before that allow-list was widened (see the CLAUDE.md entry for this feature) — already covered by that same fix, verified again here.
