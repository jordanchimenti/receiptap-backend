...
async function put(folder, file, { prefix = '' } = {}) {
  ...
  const { error } = await sb.storage.from(BUCKET).upload(key, file.buffer, {
    ...
    // cached -- so the default left a deleted receipt photo publicly readable
    // at its exact URL for an hour after someone asked for it to be gone.
    // These images are opened rarely (a receipt is looked at, not browsed), so
    // a shorter cache costs almost nothing and shrinks that window twelve-fold.
    cacheControl: '300',
  });
