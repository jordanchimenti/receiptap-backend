...
// routes/billing.js's profile-photo upload. Same caveat applies: this
// needs to move to real object storage before the app is deployed
// anywhere, since most hosts wipe local files on every redeploy.
const RECEIPT_SCAN_DIR = path.join(__dirname, '..', 'public', 'uploads', 'receipt-scans');
fs.mkdirSync(RECEIPT_SCAN_DIR, { recursive: true });

const ALLOWED_SCAN_TYPES = ['image/png', 'image/jpeg', 'image/webp'];

const uploadReceiptScan = multer({
  storage: multer.diskStorage({
    destination: RECEIPT_SCAN_DIR,
    filename: (req, file, cb) => {
      const ext = path.extname(file.originalname).toLowerCase();
      cb(null, `${req.session.customerId}-${Date.now()}${ext}`);
    },
  }),
...
router.get('/account/receipts', requireCustomerAuth, async (req, res) => {
  ...
  const [customer, transactions, scannedReceipts, monthTransactions, monthScanned, categoryRows, scannedCategoryRows,
         tappedCount, scannedCount, loyaltyCount] = await Promise.all([
    ...
    // Setup-progress counts -- deliberately NOT the filtered lists above, or
    // searching for something would appear to undo a completed step.
    ...
    prisma.loyaltyCard.count({ where: { customerId: req.session.customerId } }),
  ]);
  ...
  const setupSteps = {
    ...
    receiptSaved: tappedCount > 0,
    receiptScanned: scannedCount > 0,
    loyaltyJoined: loyaltyCount > 0,
  };
...
router.post('/account/receipts/scan', requireCustomerAuth, handleReceiptScanUpload, async (req, res) => {
  ...
  const imageUrl = `/uploads/receipt-scans/${req.file.filename}`;
...
router.post('/account/receipts/scan/confirm', requireCustomerAuth, async (req, res) => {
  ...
  if (!imageUrl || !merchantName || !total) {
    return res.render('scan-receipt-review', {
      imageUrl,
      couldNotRead: false,
      ...
      categories: CATEGORIES,
      error: 'Merchant name and total are required.',
    });
  }
  ...
  await prisma.scannedReceipt.create({
    data: {
      customerId: req.session.customerId,
      imageUrl,
